...
const Tabs = (props) => {
  ...
  useEffect(() => {
    ...
    if(props.items != "")
    {
      ...
      ///21-10-2021 na switchen tussen subtypes kan er geen actief item zijn zet dan de eerste tab op actief
      if(notOneVisible && newState.items.length > 0){
        newState.active_item = 0;
        newState.items[0].visible = true;
      }
      setState(newState);
    }
  }, []);
  ...
  function deleteItem(index) {
    ...
    let newActiveItemIndex = (clonedState.items.length - 1) >= 0 ? clonedState.items.length - 1:0
    toggleVisibility(newActiveItemIndex);
      ...
      <div className="tabs">
        { state.items.map((item, index) =>
          <div key={index} className={"item " + (item.visible == true ? 'content_visible':'')}>
            <div onClick={() => toggleVisibility(index)} className="title">
              <ContentEditable
        ...
        { state.items.map((item, index) =>
          <div key={index} className={"item " + (item.visible == true ? 'content_visible':'')}>
            ...
            <div className="editor_holder">
              ...
              <br/>
              <AddImage image={item.image} showMediaLibrary={props.showMediaLibrary} index={props.index} deleteImage={deleteImage} />

            </div>
            <div className="accordion_actions">
              {index != 0 ? <div>
                <span className="btn grey swap" onClick={() => swapItem(index, index - 1)} data-tip={t("Tab naar links")}>
                  <i className="fa fa-chevron-left"></i>
                </span>
              </div>:''}
              {index != state.items.length - 1 ? <div>
                <span className="btn grey swap" onClick={() => swapItem(index, index+1)} data-tip={t("Tab naar rechts")}>
