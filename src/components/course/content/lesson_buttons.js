...
const Buttons = (props) => {
  ...
  useEffect(() => {
    ...
    if (activeLesson !== 0 && intervention.settings.selfhelp.lessons.length > 0)
    {
      //lesson zoeken
      let lesson = intervention.settings.selfhelp.lessons.find((lesson) => {
      ...
      if (typeof lesson !== 'undefined') //kan wanneer activeLesson al is geupdate maar activeSublesson nog niet
      {
        ...
        let lessonKey = -1;
        for (const [index, lesson] of intervention.settings.selfhelp.lessons.entries()) {
        ...
        if (!(parseInt(lesson.parent_id) === 0 && parseInt(lesson.sub_id) === 0) && lessonKey !== 0)
        {
          ...
          {
            setPrevLesson(intervention.settings.selfhelp.lessons[lessonKey - 1].id);
            setPrevSubLesson(intervention.settings.selfhelp.lessons[lessonKey - 1].sub_id);
          }
          else
          {
        ...
        if (parseInt(lesson.parent_id) === 0)
        {
          ...
          if (typeof intervention.settings.selfhelp.lessons[lessonKey + 1] !== 'undefined' && (parseInt(lesson.id) === parseInt(intervention.settings.selfhelp.lessons[lessonKey + 1].parent_id) || parseInt(intervention.settings.selfhelp.lessons[lessonKey + 1].sub_id) > 0))
          {
            setNextLesson(intervention.settings.selfhelp.lessons[lessonKey + 1].id);
            setNextSubLesson(intervention.settings.selfhelp.lessons[lessonKey + 1].sub_id);
          }
          else
          {
            setCloseLesson(true);
        ...
        {
          if (typeof intervention.settings.selfhelp.lessons[lessonKey + 1] !== 'undefined')
          {
            //heeft volgende les dezelfde parent als huidige
            if (parseInt(lesson.parent_id) === parseInt(intervention.settings.selfhelp.lessons[lessonKey + 1].parent_id))
            {
              setNextLesson(intervention.settings.selfhelp.lessons[lessonKey + 1].id);
              setNextSubLesson(intervention.settings.selfhelp.lessons[lessonKey + 1].sub_id);
            }
  ...
  const endLesson = () => {
    ...
    if (typeof currentLessonAnswers === 'undefined')
    {
      ...
      dispatch(setAnswersLessons(newAllAnswers.intervention_id, newAllAnswers.answers)); /// we gaan door naar de call maar daar dispatchen we ook? deze zou dan weg kunnen.
    }
    else
    {
    ...
    if (!finished) // of altijd tbv logging?
    {
      ...
      //// check if is last lesson course..... yes then confetti
      if(intervention.settings.selfhelp.lessons[intervention.settings.selfhelp.lessons.length - 1].id == activeLesson && (typeof intervention.settings.selfhelp.alternative_menu === 'undefined' || intervention.settings.selfhelp.alternative_menu === false)){
        lastlesson = true;
  ...
  const requiredFieldsChecked = () => {
    ...
    //huidige les bepalen
    let currentLesson = intervention.settings.selfhelp.lessons.find((lesson) => {
      return parseInt(lesson.id) === parseInt(activeLesson) && parseInt(lesson.sub_id) === parseInt(activeSubLesson)
    });
    ...
    if (lessonRequiredQuestions.length > 0)
    {
      ...
      {
        if (parseInt(activeLesson) !== parseInt(nextLesson)) {
          endLesson();
        }
        return true;
      }
  ...
  const getRouting = (currentLessonKey) => {
    ...
    if (typeof currentLessonKey !== 'undefined') //zou nooit undefined moeten kunnen zijn...
    {
      ...
      if (typeof intervention.settings.selfhelp.lessons[currentLessonKey].routing !== 'undefined' && intervention.settings.selfhelp.lessons[currentLessonKey].routing.length > 0)
      {
        route_id = intervention.settings.selfhelp.lessons[currentLessonKey].routing;
      }
      else
      {
        //door alle parts van les heen loopen, bevat deze een radio of ee select vraag?
        intervention.settings.selfhelp.lessons[currentLessonKey].settings.parts.forEach(part => {
  ...
  const changeLesson = (lesson_id, lesson_sub_id, direction, check_required) => {
    ...
    if (!check_required || requiredFieldsChecked())
    {
      if (direction === 'next' && parseInt(activeLesson) === parseInt(lesson_id))
      {
        ...
        let currentLessonKey = intervention.settings.selfhelp.lessons.findIndex(lesson => {
          return parseInt(lesson.id) === parseInt(activeLesson) && parseInt(lesson.sub_id) === parseInt(activeSubLesson)
        })
  ...
  return (
    <div className="center pageControls">
      ...
      {
        closeLesson ? <span className="btn btn-primary next" onClick={() => changeLesson(0, 0, '', true)}>{t("Afronden les")}</span> : ''
      }
