...
import Buttons from './questionnaire_buttons.js';
...
const QuestionnaireContent = (props) => {
  ...
  const saveQuestionnaireAnswers = () => {
    ...
    if (answers !== undefined && !(allAnswers.answers[0].hasOwnProperty("finished") && allAnswers.answers[0].finished === true)) //om te voorkomen dat bij initialisatie van component er een leeg object gesaved wordt, of nadat deze al eerder was afgerond...
    {
      ...
      apiCall({
        action: "save_questionnaire_on_invite",
        data: {
          ic: props.ic,
          dtc: props.dtc,
          questionnaire
  ...
  return (
    <form>
      ...
      <br /><br />
      <Buttons saveImmediately={saveImmediately} activeSubLesson={activeSubLesson} ic={props.ic} dtc={props.dtc} setFinished={props.setFinished} />
    </form>
  )
