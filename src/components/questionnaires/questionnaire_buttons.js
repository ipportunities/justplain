...
import { setActiveLesson, setActiveSubLesson, setPreviousLessons } from "../../actions";
...
const Buttons = (props) => {
  ...
  const endLesson = () => {
    ...
    apiCall({
      action: "end_questionnaire_on_invite",
      data: {
        ic: props.ic,
        dtc: props.dtc
      }
  ...
  return (
    <div className="center pageControls">
      {
        parseInt(prevSubLesson) > -1 ? <span className="btn prev" onClick={() => changeLesson(prevLesson, prevSubLesson, 'prev', false)}>{t("Vorige")}</span>
        :
        <span className="btn prev" onClick={() => props.goToStep(3)}>{t("Vorige")}</span>
      }
      {
